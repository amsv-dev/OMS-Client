...
if [[ "$OBSERVABILITY_MODE" == "distributed-collectors" ]]; then
  DB_REMOTE_COLLECTORS_ENABLED=true
else
  LOGICAL_REMOTE_COLLECTOR_SHARDS=1
fi
...
LOGICAL_REMOTE_COLLECTOR_SHARDS=$LOGICAL_REMOTE_COLLECTOR_SHARDS
